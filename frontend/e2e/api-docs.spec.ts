...
test('advertises desktop downloads, PSF Guard, Python, N.I.N.A., and Siril on the home page', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Download for Windows' })).toHaveAttribute('href', 'https://github.com/theatrus/seiza-win/releases/latest')
  await expect(page.getByRole('heading', { name: 'Use Seiza in the tools you already have.' })).toBeVisible()
...
test('documents the public, catalog, compatibility, and worker APIs', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'latest release' })).toHaveAttribute('href', 'https://github.com/theatrus/seiza-win/releases/latest')
