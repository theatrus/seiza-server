  ...
  return <main className="api-docs-page">
    ...
    <div className="api-layout">
      ...
      <div className="api-content">
        ...
        <DocSection id="integrations" eyebrow="APPLICATION INTEGRATIONS" title="Desktop apps, Photoshop, PSF Guard, Python, N.I.N.A., and Siril.">
          ...
          <div className="api-note"><strong>Quick FITS browsing, including Finder</strong><span>The app bundles a FITS Quick Look preview extension for Finder and other system Quick Look clients, so inspecting a frame does not require opening or importing it first. Download the current app from <a href="https://github.com/theatrus/seiza-mac/releases/latest">Seiza for macOS releases</a>, or install it with <a href="https://github.com/theatrus/homebrew-seiza">Homebrew</a>: <code>brew install --cask theatrus/seiza/seiza-mac</code>. Previewing does not need catalogs; solving uses a complete catalog directory selected in the app’s Settings.</span></div>
          <h3>Seiza for Windows: GPU-backed viewing and local solving</h3>
          <p><a href="https://github.com/theatrus/seiza-win">Seiza for Windows</a> opens FITS, XISF, and common image files in a WinUI 3 app with a GPU-backed viewport. It provides live stretches and processing, image stacks, headers, statistics, histograms, local plate solving, catalog overlays, and full-size image exports. Download the current self-contained x64 MSI from the <a href="https://github.com/theatrus/seiza-win/releases/latest">latest release</a>. It requires Windows 11 24H2 or newer.</p>
          <h3 id="photoshop">Photoshop: open and save FITS and XISF</h3>
          <p>The <a href="https://github.com/theatrus/xisf-photoshop">FITS and XISF Photoshop plugins</a> run Seiza’s image codecs locally in Photoshop. Import as Float32 or rescaled 16-bit integer, optionally debayer Bayer mosaics to RGB, and save at the document’s 16- or 32-bit depth by default. They work without a Seiza Server account or catalog download.</p>
          <p><a href="https://github.com/theatrus/xisf-photoshop/releases/latest">Download both formats</a> as a Windows x64 installer or plugin ZIP, or a Developer ID signed and notarized universal macOS DMG. Close Photoshop before installing. On macOS, open the DMG and drag both plugin bundles onto its <strong>Photoshop Plug-ins</strong> shortcut, then eject it and restart Photoshop. A Mac ZIP remains available for manual installation. Windows plugins and the installer are signed by StackFoundry LLC starting with 0.5.0. Import defaults are under <strong>Help → About Plug-In → FITS/XISF</strong> on Windows and <strong>Photoshop → About Plug-In → FITS/XISF</strong> on macOS.</p>
          <div className="api-note"><strong>Metadata follows the document</strong><span>Starting with 0.5.0, same-format saves retain source headers and metadata; switching formats copies compatible FITS keywords on a best-effort basis. Keep the scientific originals: 16-bit import rescales pixels and loses absolute scale and precision. Re-solve astrometry after geometric edits, and use PSD/PSB to retain layers. See the <a href="https://github.com/theatrus/xisf-photoshop#metadata-retention">metadata retention limits</a>.</span></div>
          ...
          <h3>Python: install the Seiza engine with pip</h3>
          <p><a href="https://pypi.org/project/seiza/"><code>pip install seiza</code></a> provides typed Python bindings for star detection, WCS fitting, hinted and blind plate solving, satellite prediction, image calibration, and batch or live stacking. Binary ABI3 wheels support every CPython from 3.9 onward on Linux x86_64 and aarch64, macOS universal2, and Windows x64. Solving and other expensive work release the GIL.</p>
          <div className="api-note"><strong>The same catalogs and results</strong><span><code>seiza.fetch_catalogs()</code> downloads verified data into the shared cache, while <code>seiza.StarCatalog.open()</code> discovers the same <code>seiza setup</code> directories and environment overrides used by the CLI. Solutions expose pixel/world transforms and FITS WCS header cards, including TAN-SIP distortion. See the <a href="https://github.com/theatrus/seiza/tree/main/seiza-py">seiza-py guide and source</a> for complete examples.</span></div>
          <h3>Install the Seiza CLI and its catalogs</h3>
          <ol className="integration-steps">
            <li><strong>Install the release.</strong><span>From the <a href="https://github.com/theatrus/seiza/releases">Seiza releases page</a>, download the latest <code>seiza-cli-…-windows-x86_64.msi</code>. The portable ZIP and Linux packages work too; use the directory-based manual setup shown below when the installer does not provide the PATH entry and catalog-setup shortcut. On macOS, <a href="https://github.com/theatrus/homebrew-seiza">Homebrew</a> builds the CLI from source: <code>brew install theatrus/seiza/seiza</code>.</span></li>
        ...
        <DocSection id="native-api" eyebrow="NATIVE JSON API" title="Jobs and durable result artifacts.">
          ...
          <div className="api-note"><strong>Predicted satellite tracks</strong><span>When one shutter-open interval and an observer location are available, annotations include WCS-clipped <code>satellite_tracks</code> plus element age, illumination, range, elevation, trail-risk, and catalog accounting. Recent captures use current CelesTrak elements; older exposures resolve epoch-matched data from the durable cache, Seiza mirror, then IAU SatChecker. The lookup is limited to 15 seconds. While the original image is retained, each track may also include a separate <code>pixel_alignment</code> result from Seiza's bounded prediction-corridor detector. Pixel support does not prove the candidate satellite identity. Unavailable orbital data leaves the solve successful with a specific <code>unavailable_reasons.satellite_tracks</code> message, while an unavailable pixel check leaves the prediction intact.</span></div>
